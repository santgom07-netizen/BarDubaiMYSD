...
--Evento
-- Validar coherencia de horas y personas
CREATE OR REPLACE TRIGGER trg_evento_validaciones_bi
BEFORE INSERT OR UPDATE ON Evento
FOR EACH ROW
BEGIN
  -- horaFin > horaInicio
  IF :NEW.horaFin <= :NEW.horaInicio THEN
    RAISE_APPLICATION_ERROR(-21001, 'horaFin debe ser mayor que horaInicio');
  END IF;

  -- numeroPersonas > 0
  IF :NEW.numeroPersonas <= 0 THEN
    RAISE_APPLICATION_ERROR(-21002, 'numeroPersonas debe ser mayor que 0');
  END IF;
END;
/
SHOW ERRORS;
...
--Evaluacion
-- Normalizar comentario en blanco a NULL y validar rangos
CREATE OR REPLACE TRIGGER trg_eval_normaliza_bi
BEFORE INSERT OR UPDATE ON Evaluacion
FOR EACH ROW
BEGIN
  IF TRIM(:NEW.comentario) IS NULL THEN
    :NEW.comentario := NULL;
  END IF;

  IF (:NEW.calificacionServicio  NOT BETWEEN 0 AND 5) OR
     (:NEW.calificacionProducto NOT BETWEEN 0 AND 5) OR
     (:NEW.calificacionAmbiente NOT BETWEEN 0 AND 5) THEN
    RAISE_APPLICATION_ERROR(-21003, 'Calificaciones deben estar entre 0 y 5');
  END IF;
END;
/
SHOW ERRORS;
...
--Turno
-- Validar horas y normalizar asistio (NULL -> 0)
CREATE OR REPLACE TRIGGER trg_turno_validaciones_bi
BEFORE INSERT OR UPDATE ON Turno
FOR EACH ROW
BEGIN
  IF :NEW.horaFin <= :NEW.horaInicio THEN
    RAISE_APPLICATION_ERROR(-21004, 'horaFin debe ser mayor que horaInicio');
  END IF;

  IF :NEW.asistio IS NULL THEN
    :NEW.asistio := 0;
  ELSIF :NEW.asistio NOT IN (0,1) THEN
    RAISE_APPLICATION_ERROR(-21005, 'asistio debe ser 0 o 1');
  END IF;
END;
/
SHOW ERRORS;
...
--Receta
-- Asegurar coherencia entre tiempo y porciones
CREATE OR REPLACE TRIGGER trg_receta_validaciones_bi
BEFORE INSERT OR UPDATE ON Receta
FOR EACH ROW
BEGIN
  IF :NEW.tiempoPreparacion <= 0 THEN
    RAISE_APPLICATION_ERROR(-21006, 'tiempoPreparacion debe ser mayor que 0');
  END IF;

  IF :NEW.porciones <= 0 THEN
    RAISE_APPLICATION_ERROR(-21007, 'porciones debe ser mayor que 0');
  END IF;
END;
/
SHOW ERRORS;
